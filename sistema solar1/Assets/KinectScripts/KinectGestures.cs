				...
				switch(gestureData.state)
				{
					case 0:  // gesture detection - phase 1
						if(jointsTracked[rightHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightShoulderIndex] &&
				   			...
				   			Mathf.Abs(jointsPos[rightHandIndex].x - jointsPos[rightShoulderIndex].x) < 0.3f &&
				   			(jointsPos[rightHandIndex].z - jointsPos[leftElbowIndex].z) < -0.3f)
						{
							...
							gestureData.progress = 0.5f;
								...
								(jointsPos[leftHandIndex].y - jointsPos[rightElbowIndex].y) > -0.3f &&
								Mathf.Abs(jointsPos[leftHandIndex].x - jointsPos[leftShoulderIndex].x) < 0.3f &&
				...
				switch(gestureData.state)
				{
					case 0:  // gesture detection - phase 1
						if(jointsTracked[rightHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightShoulderIndex] &&
						   (jointsPos[rightHandIndex].y - jointsPos[leftElbowIndex].y) > -0.2f &&
						   Mathf.Abs(jointsPos[rightHandIndex].x - jointsPos[rightShoulderIndex].x) < 0.3f &&
						   (jointsPos[rightHandIndex].z - jointsPos[leftElbowIndex].z) < -0.45f)
						{
							SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
							gestureData.progress = 0.6f;
						}
						else if(jointsTracked[leftHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[leftShoulderIndex] &&
						        (jointsPos[leftHandIndex].y - jointsPos[rightElbowIndex].y) > -0.2f &&
						        Mathf.Abs(jointsPos[leftHandIndex].x - jointsPos[leftShoulderIndex].x) < 0.3f &&
						        (jointsPos[leftHandIndex].z - jointsPos[rightElbowIndex].z) < -0.4f)
						{
							SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
							gestureData.progress = 0.6f;
						}
					...
					case 1:  // gesture phase 2 = complete
						if((timestamp - gestureData.timestamp) < 1.6f)
						{
							bool isInPose = gestureData.joint == rightHandIndex ?
								jointsTracked[rightHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightShoulderIndex] &&
								(jointsPos[rightHandIndex].y - jointsPos[leftElbowIndex].y) > -0.2f &&
